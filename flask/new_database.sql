...
-- 資料示例form06（肥料施用紀錄）
INSERT INTO form06 (date_used, field_code, crop, fertilizer_type, material_code_or_name, fertilizer_amount, dilution_factor, operator, process, notes)
VALUES 
    ('2025-02-01', 'F000-0000', '高麗菜', '基肥', 'M000-0000', 10.00, NULL, '王小明', '間作及敷蓋稻草', '注意施肥均勻'),
    ('2025-03-15', 'F000-0001', '小白菜', '追肥', 'ooxx資材', 15.00, 0.5, '李小華', '進行追肥', '施肥後進行灌溉');
-- 查詢某田區的所有肥料施用紀錄
...
-- form07（肥料資材與代碼對照表）
CREATE TABLE form07 (
    fertilizer_material_code VARCHAR(20) PRIMARY KEY,  -- 肥料資材代碼
    fertilizer_material_name VARCHAR(100) NOT NULL,     -- 肥料資材名稱
    notes                 TEXT,                         -- 備註
    created_at            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at            TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
--  資料示例form07（肥料資材與代碼對照表）
INSERT INTO form07 (fertilizer_material_code, fertilizer_material_name, notes)
VALUES 
    ('M000-0000', 'ooxx資材', '備註'),
    ('M000-0001', 'yyzz資材', '適合高濃度施用');
-- 查詢肥料資材代碼及名稱
SELECT fertilizer_material_code, fertilizer_material_name, notes
FROM form07
WHERE fertilizer_material_code = 'M000-0000';
...
--  資料示例form08（肥料入出庫紀錄）
INSERT INTO form08 (material_name, manufacturer, supplier, packaging_unit, packaging_volume, date, purchase_quantity, usage_quantity, remaining_quantity, notes)
VALUES 
    ('ooxx資材', '某某廠商', '某某供應商', '包', '10公斤', '2025/02/05', 100.00, 10.00, 90.00, '無');
-- 查詢肥料入出庫紀錄
SELECT id, material_name, manufacturer, supplier, packaging_unit, packaging_volume, date, purchase_quantity, usage_quantity, remaining_quantity, notes
FROM form08
WHERE material_name = 'ooxx資材';
...
INSERT INTO form09 (date_used, field_code, crop, pest_target, material_code_or_name, water_volume, chemical_usage, dilution_factor, safety_harvest_period, operator_method, operator, notes)
VALUES 
    ('2025/02/05', 'F000-0000', '高麗菜', '蟲', 'M000-0000', 10.00, 0.5, 2.4, 14, '噴灑', '王小明', '無');
-- 查詢有害生物防治或環境消毒資材施用紀錄
...
-- form10（防治資材與代碼對照表）
CREATE TABLE form10 (
    pest_control_material_code VARCHAR(100) PRIMARY KEY,   -- 防治資材代碼
    pest_control_material_name VARCHAR(100) NOT NULL,       -- 防治資材名稱
    notes TEXT,                                              -- 備註
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,          -- 建立時間
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP -- 更新時間
);
-- 資料示例 form10（防治資材與代碼對照表）
INSERT INTO form10 (pest_control_material_code, pest_control_material_name, notes)
VALUES 
    ('M000-0000', 'ooxx資材', '用於防治蟲害，間作及敷蓋稻草');
-- 查詢防治資材代碼及名稱
SELECT pest_control_material_code, pest_control_material_name, notes
FROM form10
WHERE pest_control_material_code = 'M000-0000';
...
INSERT INTO form11 (material_name, dosage_form, brand_name, supplier, packaging_unit, packaging_volume, date, purchase_quantity, usage_quantity, remaining_quantity)
VALUES 
    ('ooxx資材', '顆粒型', '某某廠', '供應商A', '包', 10.0, '2025-02-05', 20.0, 10.0, 10.0);
-- 查詢有害生物防治或環境消毒資材入出庫紀錄
SELECT id, material_name, dosage_form, brand_name, supplier, packaging_unit, packaging_volume, date, purchase_quantity, usage_quantity, remaining_quantity
FROM form11
WHERE material_name = 'ooxx資材';
...
-- 資料示例form12（其他資材使用紀錄）
INSERT INTO form12 (date_used, field_code, crop, material_code_or_name, usage_amount, operator, notes)
VALUES 
    ('2025-02-05', 'F000-0000', '高麗菜', 'M000-0000/ooxx資材', 10.0, '王小明', '間作及敷蓋稻草');
--  查詢其他資材使用紀錄
SELECT id, date_used, field_code, crop, material_code_or_name, usage_amount, operator, notes
FROM form12
WHERE crop = '高麗菜';

-- form13（其他資材與代碼對照表）
CREATE TABLE form13 (
    other_material_code VARCHAR(50) PRIMARY KEY,   -- 其他資材代碼，作為主鍵
    other_material_name VARCHAR(255) NOT NULL,      -- 其他資材名稱
    notes TEXT,                                     -- 備註
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 建立時間
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP -- 更新時間
);
INSERT INTO form13 (other_material_code, other_material_name, notes)
VALUES 
    ('M000-0000', 'ooxx資材', '間作及敷蓋稻草');
SELECT other_material_code, other_material_name, notes
FROM form13
WHERE other_material_name = 'ooxx資材';
...
INSERT INTO form14 (id, material_name, manufacturer, supplier, packaging_unit, packaging_volume, date, purchase_quantity, usage_quantity, remaining_quantity)
VALUES 
    ('0000-0000', 'ooxx資材', '廠商', '供應商', '包', '10公斤', '2025/02/05', 10.00, 5.00, 5.00);
SELECT id, material_name, manufacturer, supplier, date, purchase_quantity, usage_quantity, remaining_quantity
FROM form14
WHERE material_name = 'ooxx資材';
...
CREATE TABLE form15 (
    id VARCHAR(50) PRIMARY KEY,                    -- 編號
    date DATE NOT NULL,                            -- 日期
    item ENUM('育苗場所', '溫/網室', '資材放置場所', '倉庫與工具間', '集貨場', '包裝場', '其他') NOT NULL, -- 項目
    operation ENUM('清潔', '保養', '維修', '其他') NOT NULL,  -- 作業內容
...
INSERT INTO form15 (id, date, item, operation, recorder, notes)
VALUES 
    ('0000-0000', '2025/02/05', '育苗場所', '清潔', '王小明', '清理育苗場所的灰塵及雜物');

SELECT id, date, item, operation, recorder, notes
FROM form15
WHERE item = '育苗場所';
...
CREATE TABLE form16 (
    id VARCHAR(50) PRIMARY KEY,                    -- 編號
    date DATE NOT NULL,                            -- 日期
    item ENUM('噴霧機', '割草機', '中耕機', '選別機', '貯藏/溫控設備', '搬運車', '冷藏車', '貨車', '供水設備', '其他') NOT NULL,  -- 項目
    operation ENUM('清潔', '保養', '維修', '校正', '其他') NOT NULL,  -- 作業內容
    recorder VARCHAR(255) NOT NULL,                -- 記錄人
    notes TEXT,                                    -- 備註
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 建立時間
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP -- 更新時間
);
INSERT INTO form16 (id, date, item, operation, recorder, notes)
VALUES ('0000-0000', '2025/02/05', '噴霧機', '保養', '王小明', '更換噴嘴，清洗濾網');
SELECT id, date, item, operation, recorder, notes
FROM form16
WHERE item = '噴霧機';
...
INSERT INTO form17 (id, harvest_date, field_code, crop_name, batch_or_trace_no, harvest_weight, process_date, post_harvest_treatment, post_treatment_weight, verification_status, verification_organization, notes)
VALUES ('0000-0000', '2025/02/05', 'F000-0000', '高麗菜', 'ABCD-EFHG-IJKL', 10.00, '2025/02/06', '清洗', 9.50, '非驗證產品', NULL, '間作及敷蓋稻草');
SELECT id, harvest_date, crop_name, post_harvest_treatment, post_treatment_weight, verification_status, notes
FROM form17
WHERE crop_name = '高麗菜';
...
INSERT INTO form18 (id, arena, process_date, item, batch_number, fresh_weight, operation, dry_weight, remarks)
VALUES ('0000-0000', '高麗菜', '2025-02-05', '高麗菜', 'ABCD-EFHG-IJKL', 10.00, '金針浸泡___溶液:濃度__%__小時，金針漂水___分鐘，日曬___小時，乾燥___度C___小時', 9.00, '我是備註區');
SELECT id, process_date, item, dry_weight, remarks
FROM form18
WHERE item = '高麗菜';
...
INSERT INTO form19 (id, package, sale_date, product_name, sales_target, batch_number, shipment_quantity, packaging_spec, label_usage_quantity, label_void_quantity, verification_status)
VALUES ('0000-0000', 'F000-0000', '2025-02-05', '高麗菜', '零售 (地點: ABC市場)', 'ABCD-EFHG-IJKL', 10.00, '包裝規格描述', 10, 0, '非驗證產品');
SELECT id, sale_date, product_name, shipment_quantity, verification_status
FROM form19
WHERE product_name = '高麗菜';
...
INSERT INTO form20 (id, checkitem, jobdate, operator_name)
VALUES ('0000-0000', '共5項', '2025-02-05', '王小明');
SELECT id, jobdate, operator_name, checkitem
FROM form20;
...
INSERT INTO form22 (id, date, customer_name, customer_phone, complaint, resolution, processor)
VALUES ('0000-0000', '2025-02-05', '王小明', '0988-888-888', '我是客訴內容區', '我是處理結果區', '王小明/2025-02-05');
SELECT id, date, customer_name, complaint, resolution
FROM form22;
...
CREATE TABLE form_fields (
    ...
    field_type ENUM('TEXT', 'NUMBER', 'DATE', 'BOOLEAN', 'JSON') NOT NULL, -- 欄位型別：文字、數字、日期、布林值、JSON
