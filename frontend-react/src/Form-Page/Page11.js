...
const Page11 = () => {
  ...
  // 根據選擇的資材名稱查詢資料
  const fetchMaterialPestControl = useCallback(async (pestcontrolMaterialName) => {
    try {
  ...
  const fetchData = useCallback(async () => {
    ...
    useEffect(() => {
      ...
      if (formData.pest_control_material_name) {
        fetchMaterialPestControl(formData.pest_control_material_name); // 查詢資材詳細資料
      }

    }, [fetchData, fetchPestcontrolOptions, fetchMaterialPestControl, navigate, userId, formData.pest_control_material_name]);
