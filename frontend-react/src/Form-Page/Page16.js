...
const Page16 = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await axios.get('http://127.0.0.1:5000/api/form16');
  ...
  try {
    ...
    if (formData.id) {
      if (isAdmin) {
        response = await axios.put(`http://127.0.0.1:5000/api/form16/${formData.id}`, {
          ...formData, 
          item,
          operation
        });
        ...
        // 新增資料
        response = await axios.post('http://127.0.0.1:5000/api/form16', {
          user_id: userId, 
          date: formData.date,
          item ,
          operation ,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:5000/api/form16/${id}`);
  ...
  const handleEdit = (record) => {
    ...
    const isOtherOperation = !['清潔',"保養","維修","校正"].includes(record.operation);
    const isOtherItem = ![
      '噴霧機', 
      '割草機', 
      '中耕機', 
  ...
  return (
    <div className="container">
      ...
      <Form onSubmit={handleSubmit}>
        <h4>表 16.器具/機械/設備之保養、維修、校正及清潔管理紀錄</h4>
        <FormField
          label="日期"
          type="date"
          name="date"
        ...
        <SelectField
          ...
          options={[
            ...
            { value: '中耕機', label: '中耕機' },
            { value: '選別機', label: '選別機' },
            { value: '貯藏/溫控設備', label: '貯藏/溫控設備' },
            { value: '搬運車', label: '搬運車' },
            { value: '冷藏車', label: '冷藏車' },
            { value: '貨車', label: '貨車' },
            { value: '供水設備', label: '供水設備' },
            { value: '其他', label: '其他' },
      ...
      <table className="table table-bordered table-hover table-responsive table caption-top">
        <caption>表 16.器具/機械/設備之保養、維修、校正及清潔管理紀錄</caption>
